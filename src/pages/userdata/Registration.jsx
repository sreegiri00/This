...
const Registration = () => {
  ...
  const handleSubmit = (values, { setSubmitting }) => {
    ...
    if (storedUser !== null) {
      const isAllReadyExisted = storedUser.map((res) => res.username == values.username).filter((res) => res == true).find((res) => res == true)
      console.log("isAllReadyExisted : ", isAllReadyExisted);
      if (isAllReadyExisted == true) {
        console.log("is exist..");
        
      }
      else{
        localStorage.setItem("users", JSON.stringify([...storedUser, values]));
        console.log("1");
        console.log("saved ...");
      }
    }
